...
export function splitHours(total, days) {
  if (days <= 0) return [];
...
export function daysToSplit(hours) {
  if (hours <= 2) return 1;
  if (hours <= 4) return 2;
  if (hours <= 6) return 3;
  if (hours <= 9) return 4;
...
export const PROTO_TODAY = new Date(2026, 3, 27);
...
export function makeInitialTasks() {
  const wk0 = weekDates(0).map(dateKey);
  const wk1 = weekDates(1).map(dateKey);
  return [
    {
...
export function fmtTimeRange(start, dur) {
  const end = start + dur;
